...
class SnapraidRunner:
    ...
    def __init__(self, config_file, scrub=None, cleanup=None, pool=None,
                 ignore_deletethreshold=False, dry_run=False, **kwargs):
        ...
        # SnapRaid-BTRFS Options
        self.snapraid_btrfs_exe = config.getstring('snapraid-btrfs', 'executable')
        self.snapraid_btrfs_cleanup = cleanup if cleanup is not None else \
        ...
        self.snapper_configs = config.getstring('snapper', 'configs', fallback=None)
        if self.snapper_configs is not None:
            self.snapper_configs = self.snapper_configs.split(',')
        self.snapper_configs_file = config.getstring('snapper', 'configs-file', fallback=None)
        ...
        self.snapraid_scrub = scrub if scrub is not None else \
    ...
    def snapraid_btrfs_command(self, command, allow_statuscodes=None, **kwargs):
        btrfs_args = [
            '--quiet', '--conf', self.snapraid_config,
            '--snapper-path', self.snapper_exe,
            '--snapraid-path', self.snapraid_exe,
        ]
        if self.snapper_configs is not None:
            btrfs_args.extend(['--snapper-configs', ','.join(self.snapper_configs)])
        if self.snapper_configs_file is not None:
            btrfs_args.extend(['--snapper-configs-file', self.snapper_configs_file])
        if command == 'pool' and self.snapraid_btrfs_pool_dir is not None:
            btrfs_args.extend(['--pool-dir', self.snapraid_btrfs_pool_dir])

        sr_args = [] if command == 'cleanup' else ['--quiet']
        for k, v in kwargs.items():
            sr_args.extend(['--' + k, str(v)])

        if self.dry_run:
            logging.info(' '.join([self.snapraid_btrfs_exe] + btrfs_args +
                                  [command] + sr_args))
            return []

        p = subprocess.Popen(
            [self.snapraid_btrfs_exe] + btrfs_args + [command] + sr_args,
        ...
        if return_code != 0:
            if allow_statuscodes is None or return_code not in allow_statuscodes:
                raise subprocess.CalledProcessError(return_code, 'snapraid-btrfs ' + command)
    ...
    def run(self):
        ...
        if not os.path.isfile(self.snapraid_btrfs_exe):
            logging.error('The configured snapraid-btrfs executable "%s" does not '
                          'exist or is not a file' % self.snapraid_btrfs_exe)
            self.finish(False)
        if not os.path.isfile(self.snapper_exe):
            logging.error('The configured snapper executable "%s" does not '
                          'exist or is not a file' % self.snapper_exe)
            self.finish(False)
        ...
        if self.snapraid_touch:
            ...
            self.snapraid_btrfs_command('touch')
            logging.info('*' * 60)

        logging.info('Running diff...')
        diff_out = self.snapraid_btrfs_command('diff', allow_statuscodes=[2])
        logging.info('*' * 60)
        ...
        if sum([diff_results[x] for x in ['remove', 'add', 'move', 'update']]) == 0:
            ...
            try:
                self.snapraid_btrfs_command('sync')
            except subprocess.CalledProcessError as e:
                logging.error(e)
                self.finish(False)
            logging.info('*' * 60)

        if self.snapraid_btrfs_pool:
            logging.info('Running pool...')
            try:
                self.snapraid_btrfs_command('pool')
            except subprocess.CalledProcessError as e:
                logging.error(e)
                self.finish(False)
            logging.info("*" * 60)

        if self.snapraid_btrfs_cleanup:
            logging.info('Running cleanup...')
            try:
                self.snapraid_btrfs_command('cleanup')
            except subprocess.CalledProcessError as e:
                logging.error(e)
                self.finish(False)
            logging.info("*" * 60)

        if self.snapraid_scrub:
            ...
            try:
                self.snapraid_btrfs_command('scrub', **scrub_args)
            except subprocess.CalledProcessError as e:
                logging.error(e)
                self.finish(False)
            logging.info('*' * 60)
